...
def viewPlans():
	...
	plans = cur.fetchall()
	cur.close()
	return render_template('viewPlans.html', plans=plans)
...
def adminDash():
	...
	cur.execute("SELECT * FROM trainors")
	trainors = cur.fetchall()

	cur.execute("SELECT name, count FROM equip")
	equips = cur.fetchall()

	cur.close()
	return render_template('adminDash.html', members=members, plans=plans, trainors=trainors, equips=equips)
...
def addMember():
	...
	q = cur.execute("SELECT username FROM info")
	b = cur.fetchall()
	all_usernames = [b[i]['username'] for i in range(q)]

	q = cur.execute("SELECT DISTINCT name FROM plans")
	b = cur.fetchall()
	plan_choices = [(b[i]['name'], b[i]['name']) for i in range(q)]

	q = cur.execute("SELECT username FROM trainors")
	b = cur.fetchall()
	trainor_choices = [(b[i]['username'], b[i]['username']) for i in range(q)]

	cur.close()

	form = AddMemberForm(request.form)
	form.plan.choices = plan_choices
	form.trainor.choices = trainor_choices
...
def edit_profile(username):
	...
	if username != session['username'] and session['prof'] not in [1, 2]:
...
if __name__ == "__main__":
	# secret_key is already set above — kept here only for clarity, not needed
